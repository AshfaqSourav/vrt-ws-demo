...
import { DemoGameOnPage } from '../../pages/Demo/DemoGameOnPage.js';
import { compareScreenshots } from '../../utils/compareScreenShots.js';
import { generateHtmlReport as generateLaptopHtml } from '../../utils/HtmlReport/Demo/GameOn/laptop.js';
import { generateHtmlReport as generateMobileHtml } from '../../utils/HtmlReport/Demo/GameOn/mobile.js';
...
const viewports = [
  {
    name: 'Laptop',
    expectedPath: './expected_screenshots/demoGameOn/demoGameOnLaptopFigma.png',
    htmlGen: generateLaptopHtml
  },

  {
    name: 'Mobile',
    expectedPath: './expected_screenshots/demoGameOn/demoGameOnMobileFigma.png',
    htmlGen: generateMobileHtml
  }
];

// SERIAL BLOCK — Ensures tests run one after another
test.describe.serial('Demo Game On VRT Suite', () => {
  for (const { name: viewport, expectedPath, htmlGen } of viewports) {
    test(`${viewport} - Demo Game On visual should match Figma`, async ({ page }) => { 
      const demoGameOn = new DemoGameOnPage(page, viewport);
      await demoGameOn.goto();
      await scrollPage(page);

      const { cropped } = await demoGameOn.takeScreenshot();
      const actualBuffer = fs.readFileSync(cropped);

      const diffPixels = compareScreenshots({
        actualBuffer,
        expectedPath,
        actualPath: `${diffDir}/demogameOn${viewport}-actual.png`,
        diffPath: `${diffDir}/demoGameOn${viewport}-diff.png`,
        expectedCopyPath: `${diffDir}/demoGameOn${viewport}-expected.png`
      });
      ...
      htmlGen({
        diffPixels,
        outputDir: diffDir,
        reportPath: `${diffDir}/demoGameOn${viewport}-report.html`,
        pageName: `Demo Game On ${viewport}`
      });
...
// };
  ...
  test.afterAll(async () => {
    const reportPath = path.resolve('./diff_output/demoGameOnMultiViewportReport.html');
  ...
  expectedImage: `demoGameOn${name}-expected.png`,
  actualImage: `demoGameOn${name}-actual.png`,
  diffImage: `demoGameOn${name}-diff.png`
}));
