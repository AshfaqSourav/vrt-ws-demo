...
import { maskEverythingExcept , cropWhiteMargins } from '../../utils/domUtils';

dotenv.config();

const viewportSizes = getEnabledViewports(4); 
const selector = '#demo-container';


export class demoDefaultStrategyPage {
  ...
  async goto() {
     await this.page.goto(`${process.env.BASE_URL}/demo`, {
...
async takeScreenshot() {
  ...
  const maskedFullPagePath = `${diffDir}/demoDefaultStraregy${this.viewport}-masked-full.png`;
  const croppedElementPath = `${diffDir}/demoDefaultStraregy${this.viewport}.png`;
